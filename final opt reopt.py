...
def funcion(lista_ordenada, lista_mermas, largo, casas, dict_, escuadrias_utilizadas, lista_completos):
    ...
    for escuadria in lista_ordenada:
        ...
        for trozo in escuadria:
            indicador = False
            if trozo > largo_escuadria and trozo < largo:  #and (trozo - largo_escuadria) > 1
            ...
            if 0.25 < probabilidad < 0.75:# prob salga bien cortado
                ...
                lista_mermas.append(largo_escuadria - trozo_malo) # manejar
                print(f"el trozo es {trozo_malo} y el largo es {largo_escuadria}, la dif es {largo_escuadria - trozo_malo}")
                lista_mermas.sort()
...
def optimizar(restantes, largo, lista_mermas):
    print(f"restantes es {restantes}")
    new_rolls = []
    new_lista_merma = []
    new_restantes = []
...
if __name__ == "__main__":
    ...
    for i in range(1):
        ...
        for key in lista_completos:
            total_cortes += key * lista_completos[key]
        
        # print(total)
        # print(f"el total sin merma es {total/320}")
        # print(f"la lista mermas es {lista_mermas}")
        ...
        print(f"el total cortes es {total_cortes} y es {total_cortes/320} escuadrias")
        print(f"el total cortes es {total_mermas} y es {total_mermas/320} escuadrias")
        total = total_cortes + total_mermas
        # print(f"el total con merma es {total/320}")
